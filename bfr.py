...
def K_means(data: list, k: int, label=0):
    ...
    # 中心點更新: 若有變更長老，就更新名單，沒有變更長老就跳過此循環
    while status_revise:
        status_revise = 0
        for i in sample:
    ...
    # 若有離散點
    if rs_dic:
        rs_list = list(rs_dic.keys()) # 離散成員名單
        print('rs_list: ', rs_list)
        cluster_result = -1 # 離散成員發給他一張卡 叫-1
...
def main():
    ...
    first_num = len(data)
    print("Total num:", first_num)  #      幾%  sample總數
    ...
    while chunk_num < len(chunk):
        ...
        # 新進移民 先分進DS，審查他是否在馬式距離內，是否有資格進部落
